...
- `pyopenjtalk` worker への socket request を直列化し、複数 client が同時に音声合成した際に response が入れ替わる問題を修正

### v2.8.6 (2026-07-13)

- Google Colab support を終了し、Colab notebook を削除
- `pyopenjtalk` worker を起動時の current directory に依存せず開始できるように修正
- GPU 検出に失敗した場合も FastAPI server の status endpoint を利用できるように修正

### v2.8.5 (2026-07-12)

- 空に近い text、全要素が 0 の waveform、line split 時の sampling rate、複数話者指定など、推論と API の境界条件を修正
- user dictionary access と checkpoint 保存を並行実行に耐えるようにし、WSL2 の分散学習 backend を `gloo` に変更
- 前処理 failure の伝播、学習再開時の learning rate fallback、style audio decode、style vector 保存など、学習 pipeline の failure handling を改善
- PyTorch package の CUDA index を `cu129` に統一し、ONNX Runtime family を CUDA 12 対応の 1.24 系に固定
- editor、FastAPI、NLP、推論、学習、変換・merge、Gradio WebUI の挙動仕様と regression test を整備
- repository 全体へ Ruff の lint、format、import sorting を適用

### v2.8.4 (2026-07-09)

- pytest marker と test path を整理し、環境依存 test を分離できる構造へ変更
- 言語処理と API の test coverage を拡充
- English BERT tokenizer 用 dependency に `sentencepiece` を追加

### v2.8.3 (2026-07-08)

- 現行 PyTorch で safetensors model を ONNX export できるように修復
- AIVM / AIVMX 変換を削除し、ONNX export path を単純化

### v2.8.2 (2026-07-08)

- training path を現行 PyTorch API に更新
- project metadata と package settings を独立 fork 向けに整理
- Ada / Blackwell 世代 GPU 対応を要件に追加

### v2.8.1 (2026-07-08)

- Python package を `src/style_bert_vits2/` の src layout へ移行
- 要件と roadmap の canonical documentation layer を追加

### v2.8.0 (2026-07-08)

- 日本語の記号、異体字、漢数字、電話番号、郵便番号、住所、部屋番号、階数表記の normalization を拡充
- 日本語 g2p の alignment edge case を修正し、metadata API を追加
- FastAPI server の port override に対応
- dependency declaration を `pyproject.toml` に集約し、再現可能な resolution 用に `uv.lock` を追加

### v2.7.1 (2026-07-07)

- `pyopenjtalk-plus` へ移行して NumPy 2 に対応し、日本語 g2p と BERT feature の smoke test を追加
- package version を git tag から導出する hatch-vcs 構成へ移行
- training dependency を uv の `train` group として定義
- training device selection と BERT model dtype の決定性を修正

## Upstream-derived release history
