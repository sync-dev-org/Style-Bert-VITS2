...
アクセント指定は日本語かつ改行分割なしの場合だけ合成へ渡される。条件を満たさない指定や JSON の解析失敗は、警告文を情報欄へ追加して tone なしで合成を続ける。`InvalidToneError` と `ValueError` の分岐は、イベントが要求する 4 出力に合わせて `Error: ...`、音声なし、元のアクセント文字列、強制再ロード state 据え置き (`gr.update()`) を返す。それ以外の例外は関数内で捕捉せず、Gradio のイベントエラーとなる。
...
空のモデル名に対する `merge_models_gr()` は、イベントが要求する情報欄とスタイル欄の 2 出力に合わせて、エラー文とスタイル欄据え置き (`gr.update()`) を返す。ファイル不在、互換性のない parameter、読み書き例外は捕捉せず、Gradio のイベントエラーとなる。
...
画面には「作るスタイルの数」、「各モデルのスタイルを取得」、方式に応じて動的に描画されるスタイル選択行、「スタイルを増やす」「スタイルを減らす」、「スタイルのマージ」がある。
